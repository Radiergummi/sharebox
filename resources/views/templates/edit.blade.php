...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form class="card" action="{{ route('templates.update', [ $template->id ]) }}" method="post">
                    {{ method_field('PUT') }}
                    {{ csrf_field() }}
                    ...
                    <header class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <h3 class="card-title template-name">
                            {{ __('templates.edit_template', ['name' => $template->name]) }}
                        </h3>
                        ...
                        <div class="card-actions">
                            <button
                                    ...
                                    v-on:click.prevent="navigate('{{ route('templates.show', [ $template->id ]) }}')"
                            >
                    ...
                    <section class="card-body">
                        <p class="help-text text-muted" v-pre>
                            {!! __('templates.edit_template_help', [ 'help' => route('help') ]) !!}
                        </p>
                        <div class="form-group">
                            ...
                            />
                            @if ($template->locked)
                                <span class="text-info">{{ __('templates.edit_default_template') }}</span>
                            @endif
                            @if ($errors->has('name'))
                        ...
                        <div class="form-group">
                            <label for="field-language" class="card-subtitle">{{ __('templates.language') }}</label>
                            <p class="text-muted">{{ __('templates.language_help') }}</p>
                            <select
                                    id="field-language"
                                    name="language"
                                    class="form-control"
                            >
                                @foreach ($languages as $language)
                                    <option value="{{ $language }}" {{ $language === $template->language ? 'selected' : '' }}>
