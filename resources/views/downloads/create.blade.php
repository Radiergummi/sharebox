...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form class="card" action="{{ route('downloads.store') }}" method="post">
                    {{ csrf_field() }}

                    <header class="card-header d-flex justify-content-between align-items-center">
                        <h3 class="card-title download-name">{{ __('downloads.create_download') }}</h3>
                                    ...
                                    <optgroup label="{{ __('languages.' . $group['language']) }}">
                                        @foreach ($group['templates'] as $template)
                                            @unless ($template->slot === 'missing' || $template->slot === 'expired')
                                                <option
                                                        value="{{ $template->id }}"
                                                        {{ old('template') === $template->id || $template->locked ? ' selected' : '' }}
                                                >
                                                    {{ $template->name }}
                                                </option>
                                            @endunless
